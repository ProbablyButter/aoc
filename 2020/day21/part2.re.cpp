...
int main(int argc, char **argv)
{
  ...
  // now to actually do the challenge: extract all dangerous ingredients and sort alphabetically (by english name)
  std::vector<std::string> allergens;
  for (auto &v : allergen_to_ingredient)
  {
    allergens.push_back(v.first);
  }
  std::sort(allergens.begin(), allergens.end());
  for (auto &v : allergens)
  {
    std::cout << *allergen_to_ingredient[v].begin() << ",";
  }
  std::cout << std::endl;
}
