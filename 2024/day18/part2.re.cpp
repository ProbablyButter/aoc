...
int main(int argc, char **argv) {
  ...
  while (num_corrupted < coords.size()) {
    grid(coords[num_corrupted].second, coords[num_corrupted].first) = 1;
    ...
    if (dist.find(stop) == dist.end()) {
      break;
    }
    ++num_corrupted;
  }
  std::cout << coords[num_corrupted].first << ","
            << coords[num_corrupted].second << std::endl;
}
